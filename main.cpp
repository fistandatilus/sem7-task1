...
int main(int argc, char *argv[]) {
    ...
    int n, m, mode;
    double mu;
    if (!(argc == 5 && sscanf(argv[1], "%d", &n) == 1 && sscanf(argv[2], "%d", &m) == 1 && sscanf(argv[3], "%lf", &mu) == 1 && sscanf(argv[4], "%d", &mode) == 1 && mode > 0 && mode <= 4)) {
    ...
    for (int j = 1; j <= 1; j++) {
        ...
        p_she.N = n*scale;
        p_she.Dim = m*scale + 1;
        p_she.h_x = p_gas.Segm_X/(m*scale);
        p_she.tau = p_gas.Segm_T/(n*scale);
        time = clock();
